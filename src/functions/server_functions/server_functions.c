...
int server_limit_public_addresses(const int SETTINGS, const char* MESSAGE)
{
  ...
  if (SETTINGS == 1 || SETTINGS == 2)
  {
    ...
    // }
    // else 
    // if (strncmp(TEST_WALLET_1,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_2,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_3,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_4,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_5,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_6,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_7,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_8,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_9,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_10,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_11,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_12,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_13,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_14,data2,BUFFER_SIZE) == 0 || strncmp(TEST_WALLET_15,data2,BUFFER_SIZE) == 0 || strncmp("XCA1pKHPMmF1KSYGdBJ2SNfrzANHtZ6aF7eDFyHSTRi653rbBnnFm7uMijexoG4djPC4T9X83q4Jbgjwxya6UUaE4MkxmkD5kJ",data2,BUFFER_SIZE) == 0 || strncmp("XCA1UArNZKkEYz6qArKgtyJimMBw5xkjE5ec33ymywWU1XvXGzZKWcqWKMjtqwfRdiNeQaeZyJJ7aZfXk3C5TMos4RfueH7ER8",data2,BUFFER_SIZE) == 0 || strncmp("XCA1cC6xQza9AUkL9Cvby49PWSr2aDMfVQi2aaNa5UsJTVddWdn7yJvBcqG2ewnDh3Ee38qWJkEdW52J96zT1C6f7awCAMYxpG",data2,BUFFER_SIZE) == 0 || strncmp("XCA1dJ2TSfwfAeaSTMSzLL83YDnNs6e1mHxkVdx8tdigMSCqqAGDjpi3bV1LCrjHQKh4N1s7kPE3bSoeh4ypwgsj1SRtfhP14K",data2,BUFFER_SIZE) == 0 || strncmp("XCA1sCezaPUYxBAiUXS6xd9kvyNxCcqA65vspMCPAB78Gjjef9E6S7FQ47yFkNA2RdCwybAk2XWhPDT9F1t6c4Jg5JkLy2Wb1p",data2,BUFFER_SIZE) == 0 || strncmp("XCA1gFTg7iiZGJa8RmvT81H66iyLcffBtTYKCRMmnnfLd4yAWcKG4khYD4ESmWGvy6MZZSaTYcvRYC6Exkgd475g3WaoU1tWcY",data2,BUFFER_SIZE) == 0)
    // {
      
    // }
    // else
    if (strncmp(network_data_nodes_list.network_data_nodes_public_address[0], data2,
                sizeof(network_data_nodes_list.network_data_nodes_public_address[0])) == 0 ||
        strncmp(network_data_nodes_list.network_data_nodes_public_address[1], data2,
                sizeof(network_data_nodes_list.network_data_nodes_public_address[1])) == 0 ||
        strncmp(network_data_nodes_list.network_data_nodes_public_address[2], data2,
                sizeof(network_data_nodes_list.network_data_nodes_public_address[2])) == 0 ||
        strncmp(network_data_nodes_list.network_data_nodes_public_address[3], data2,
                sizeof(network_data_nodes_list.network_data_nodes_public_address[3])) == 0 ||
        strncmp(network_data_nodes_list.network_data_nodes_public_address[4], data2,
                sizeof(network_data_nodes_list.network_data_nodes_public_address[4])) == 0) {
    } else {
        // check to make sure that the public address is registered to a delegate and the delegate has the
        // DATABASE_DATA_SYNC_DELEGATE_MINIMUM_AMOUNT
        memset(data2, 0, sizeof(data2));
        if (read_document_field_from_collection(database_name, DATABASE_COLLECTION, data3, "total_vote_count", data2) ==
            1) {
